...
export default function OperatingNavigator() {
  ...
  // Retain the canonical source reference as an explicit contract for shell regression tests.
  void primaryNavSections;
  ...
  return (
    <nav aria-label={t("intelligenceSpaces.operatingNavigator")} className="space-y-4">
        ...
        {navigationCenterItems.map((item) => (
          <li key={item.href}>
            <NavRow
              ...
              icon={item.icon}
              live={hydrated && activeCenterHref === item.href ? "active" : "neutral"}
              t={t}
              spatial={isHome}
            />
          </li>
      ...
      <details
        className="cbai-nav-disclosure border-t border-[var(--cbai-border-subtle)] pt-3"
