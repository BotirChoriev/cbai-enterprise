    ...
    <div className="space-y-3">
      <div>
        <h3 className="text-sm font-semibold text-zinc-100">
          {t("researchTopicCompletion.negativeResultsTitle")}
        </h3>
        <p className="mt-1 text-xs text-zinc-500">
          {t("researchTopicCompletion.negativeResultsNotConnected")}
        </p>
      </div>
      ...
      <div className={`${cbaiGlassCard} space-y-3 p-3`}>
        ...
        <dl className="space-y-2 text-[11px]">
          ...
          <div>
            <dt className="font-medium uppercase tracking-wider text-zinc-600">
              {t("researchTopicCompletion.futureWorkspaceSupport")}
            </dt>
            <dd className="mt-0.5 text-zinc-500">
              {t("researchTopicCompletion.futureExperimentTypes")}:{" "}
              {readiness.futureExperimentTypes.join(" · ")}
            </dd>
            <dd className="mt-1 text-zinc-500">
              {t("researchTopicCompletion.futureEvidence")}:{" "}
              {readiness.futureEvidenceSources.join(" · ")}
            </dd>
          </div>
        </dl>
        ...
        {readiness.humanReviewRequired ? (
          <p className="border-t border-zinc-800/80 pt-2 text-[10px] text-zinc-600">
            {t("researchTopicCompletion.negativeHumanReview")}
          </p>
        ) : null}
      </div>
      ...
      {!isTopicSpecific ? (
        <p className="text-[11px] text-zinc-600">
          {t("researchTopicCompletion.genericNegativeResults")}
        </p>
      ) : null}
    </div>
  );
