...
function EvidenceReadinessCard({ title, status, supported, limitation }: EvidenceCardProps) {
  return (
    <article className={`${cbaiGlassCard} flex flex-col p-4`}>
      ...
      <dl className="mt-3 flex-1 space-y-3 text-xs">
        ...
        <div>
          <dt className="font-medium uppercase tracking-wider text-zinc-600">Will support</dt>
          <dd className="mt-1 leading-relaxed text-zinc-400">{supported}</dd>
        </div>
        <div>
          <dt className="font-medium uppercase tracking-wider text-zinc-600">Current limitation</dt>
      ...
      <p className="mt-4 border-t border-zinc-800/80 pt-3 text-[11px] text-zinc-600">
        Human review required before any future use in decisions.
      </p>
    </article>
  );
}
    ...
    <section aria-labelledby="topic-evidence-readiness-heading" className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Core research evidence areas for this topic — not connected yet. Available catalog
          information only.
        </p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <EvidenceReadinessCard
          ...
          limitation={publication.layer.limitations[0] ?? "Publication sources are not connected yet."}
        />
        <EvidenceReadinessCard
          title="Experiments"
          ...
          limitation={experiment.layer.limitations[0] ?? "Experiment records are not connected yet."}
        />
        <EvidenceReadinessCard
          title="Laboratories"
          ...
          limitation={laboratory.layer.limitations[0] ?? "Laboratory records are not connected yet."}
        />
      </div>
    </section>
  );
