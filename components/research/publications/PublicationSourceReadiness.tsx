...
type PublicationSourceReadinessProps = {
  layer: PublicationLayer;
};

export default function PublicationSourceReadiness({ layer }: PublicationSourceReadinessProps) {
  return (
    <div className="space-y-4">
      <div className={`${cbaiGlassCard} grid gap-4 p-4 sm:grid-cols-2`}>
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-300">
            {PUBLICATION_LAYER_SOURCE_STATUS_LABELS[layer.sourceStatus]}
          </p>
        </div>
        <div>
          <p className="text-[10px] font-medium uppercase tracking-wider text-zinc-600">
          ...
          <p className="mt-1 text-sm text-zinc-300">
            {PUBLICATION_LAYER_EVIDENCE_STATUS_LABELS[layer.evidenceStatus]}
          </p>
        </div>
      </div>
      ...
      {layer.humanReviewRequired ? (
        <p className="text-xs text-zinc-600">
          Human review required before any future publication metadata supports a decision.
        </p>
      ) : null}
    </div>
  );
