...
type LaboratoryReadinessProps = {
  layer: LaboratoryLayer;
};

export default function LaboratoryReadiness({ layer }: LaboratoryReadinessProps) {
  return (
    <div className="space-y-4">
      <div className={`${cbaiGlassCard} grid gap-4 p-4 sm:grid-cols-2`}>
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-300">
            {LABORATORY_LAYER_SOURCE_STATUS_LABELS[layer.sourceStatus]}
          </p>
        </div>
        <div>
          <p className="text-[10px] font-medium uppercase tracking-wider text-zinc-600">
          ...
          <p className="mt-1 text-sm text-zinc-300">
            {LABORATORY_LAYER_EVIDENCE_STATUS_LABELS[layer.evidenceStatus]}
          </p>
        </div>
      </div>
      ...
      <div className={`${cbaiGlassCard} grid gap-4 p-4 sm:grid-cols-3`}>
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-300">
            {layer.equipmentSupported ? "Planned — not connected yet" : "Not supported in this layer"}
          </p>
        </div>
        <div>
          <p className="text-[10px] font-medium uppercase tracking-wider text-zinc-600">
            Safety / ethics support
          </p>
          <p className="mt-1 text-sm text-zinc-300">
            {layer.safetyEthicsSupported
      ...
      {layer.humanReviewRequired ? (
        <p className="text-xs text-zinc-600">
          Human review required before any future laboratory metadata supports a decision.
        </p>
      ) : null}
    </div>
  );
