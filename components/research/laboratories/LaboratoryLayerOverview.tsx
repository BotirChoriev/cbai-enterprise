...
export default function LaboratoryLayerOverview() {
  const labTypes = listLaboratoryTypes();
  const metadataFields = listExpectedLaboratoryMetadataFields();

  return (
    <div className="space-y-4">
      <div>
        <p className={cbaiSectionEyebrow}>Laboratories layer</p>
        <h3 className="text-base font-semibold text-zinc-100">Laboratory records readiness</h3>
        <p className="mt-1 text-sm text-zinc-500">
          Laboratories are a core research object. Laboratory records and metadata are not connected
          ...
          <ul className="mt-3 grid gap-1.5 sm:grid-cols-2">
            {metadataFields.map((field) => (
              <li key={field} className="text-xs text-zinc-400">
                {LABORATORY_EXPECTED_METADATA_LABELS[field]}
              </li>
            ))}
          </ul>
